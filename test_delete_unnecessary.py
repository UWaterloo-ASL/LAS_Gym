...
# -*- coding: utf-8 -*-
"""
Created on Mon May 14 00:18:55 2018
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        actor = ActorNetwork(sess, observation_space, action_space,
                             float(args['actor_lr']), float(args['tau']),
                             int(args['minibatch_size']))

        critic = CriticNetwork(sess, observation_space, action_space,
                               float(args['critic_lr']), float(args['tau']),
                               float(args['gamma']),
                               actor.get_num_trainable_vars())
        
        actor_noise = OrnsteinUhlenbeckActionNoise(mu=np.zeros(action_space.shape[0]))
